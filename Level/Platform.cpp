...
namespace Platform {
    ...
    auto calculateDistance2D(const vec2f &p1, const vec2f &p2) -> f32 {
        return (f32) (sqrt(pow((p1.x - p2.x), 2) + pow((p1.y - p2.y), 2)));
    }

    static inline
    auto renderBoundingVolume(vec2f position) -> std::tuple<i32, i32, i32, i32> {
        return {
                -RENDER_RADIUS * CHUNK_SIZE + position.x,
                 RENDER_RADIUS * CHUNK_SIZE - CHUNK_SIZE + position.x,
                -RENDER_RADIUS * CHUNK_SIZE + position.y,
                 RENDER_RADIUS * CHUNK_SIZE - CHUNK_SIZE + position.y
        };
    }
    ...
    auto Platform::tick(Camera::Camera& camera) -> void {
        ...
        const auto nCurrentRoot  = glm::vec2 {
            ((i32) (camera.getCameraPosition().x)) % CHUNK_SIZE > CHUNK_SIZE / 2
            ? ((i32) (camera.getCameraPosition().x) / CHUNK_SIZE) * CHUNK_SIZE + CHUNK_SIZE
            : ((i32) (camera.getCameraPosition().x) / CHUNK_SIZE) * CHUNK_SIZE,
            ((i32) (camera.getCameraPosition().z)) % CHUNK_SIZE > CHUNK_SIZE / 2
            ? ((i32) (camera.getCameraPosition().z) / CHUNK_SIZE) * CHUNK_SIZE + CHUNK_SIZE
            : ((i32) (camera.getCameraPosition().z) / CHUNK_SIZE) * CHUNK_SIZE
        };
