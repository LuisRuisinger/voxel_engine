...
namespace core::level::chunk {
    ...
    auto Chunk::insert(const vec3f position, u8 voxelID, Platform *platform) -> void {
    ...
    auto Chunk::find(vec3f position, Platform *platform) -> std::pair<octree::Node *, ChunkData> {
    ...
    auto Chunk::generate(Platform *platform) -> void {
        auto fun = [this](const vec3f position, u8 voxelID, Platform *platform) -> void {
            auto  normalizedVec = CHUNK_SEGMENT_YNORMALIZE(position);
            auto &segment       = _chunksegments[CHUNK_SEGMENT_YDIFF(position)];
